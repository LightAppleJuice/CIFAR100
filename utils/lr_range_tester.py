# Fixed version of lr_finder from
...
class LRFinder(object):
    ...
    def __init__(
        ...
        self.history = {"lr": [], "loss_train": [], "loss_val": []}
        self.best_loss = None
    ...
    def plot(self, skip_start=10, skip_end=5, log_lr=True, show_lr=None, ax=None):
        ...
        lrs = self.history["lr"]
        losses = self.history["loss_train"]
        losses_val = self.history["loss_val"]
        if skip_end == 0:
            lrs = lrs[skip_start:]
            losses = losses[skip_start:]
            if losses_val:
                losses_val = losses_val[skip_start:]

        else:
            lrs = lrs[skip_start:-skip_end]
            losses = losses[skip_start:-skip_end]
            if losses_val:
                losses_val = losses_val[skip_start:-skip_end]
